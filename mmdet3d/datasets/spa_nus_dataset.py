...
cls_label_map = {'car' : 'car', 'motorcycle': 'motorcycle', 'pedestrian': 'pedestrian', 'bicycle':'bicycle'}

@DATASETS.register_module()
class SPA_Nus_Dataset(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def get_data_info(self, index):
        ...
        cam_orders = ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_BACK_RIGHT', 'CAM_BACK_LEFT']
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                extrinsic = cam_info['cam_intrinsic'][1][:3, :3]
                translation = cam_info['cam_intrinsic'][1][:3, 3].reshape(3, 1)
                projection_matrix = np.dot(intrinsic, extrinsic)
                projection_matrix = np.vstack( (np.hstack((projection_matrix, translation)), np.array([0,0,0,1])))
                
                # viewpad = np.eye(4)
                # viewpad[:intrinsic.shape[0], :intrinsic.shape[1]] = intrinsic
                # lidar2img_rt = (viewpad @ lidar2cam_rt.T)
                # lidar2img_rts.append(lidar2img_rt)
                lidar2img_rts.append(projection_matrix)
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                        ...
                        attr = 'vehicle.moving'
                    elif name in ['motorcycle', 'cyclist']:
                        attr = 'cycle.with_rider'
                    else:
                        attr = SPA_Nus_Dataset.DefaultAttribute[name]
                else:
...
def output_to_nusc_box(detection, with_velocity=False):
...
class SPA_NuScenesEval(DetectionSPAEval):
    ...
    def evaluate(self) -> Tuple[DetectionMetrics, DetectionMetricDataList]:
        ...
        if self.verbose:
            print('Accumulating metric data...')
        metric_data_list = DetectionMetricDataList()
        self.cfg.class_names = ['car', 'pedestrian', 'motorcycle', 'bicycle']
        for class_name in self.cfg.class_names:
            for dist_th in self.cfg.dist_ths:
                md = accumulate(self.gt_boxes, self.pred_boxes, class_name, self.cfg.dist_fcn_callable, dist_th)
                metric_data_list.set(class_name, dist_th, md)
