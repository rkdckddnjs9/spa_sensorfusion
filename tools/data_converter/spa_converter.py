...
from mmdet3d.core.bbox import box_np_ops_spa, points_cam2img
from .spa_data_utils import WaymoInfoGatherer, get_spa_image_info
from .nuscenes_converter import post_process_coords

spa_categories = ('Pedestrian', 'Cyclist', 'Car', 'Motorcyclist')


def convert_to_spa_info_version2(info):
    """convert spa info v1 to v2 if possible.

    Args:
        info (dict): Info of the input spa data.
...
class _NumPointsInGTCalculater:
    ...
    def calculate_single(self, info):
        ...
        if self.remove_outside:
            points_v = box_np_ops_spa.remove_outside_points(
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        if remove_outside:
            ...
            #     # points_v = box_np_ops_spa.remove_outside_points(
            #     #     points_v, rect, Trv2c, P, image_info['image_shape'][ii])
            #     # points_v_.append(points_v)
            #     points_v_ = box_np_ops_spa.remove_outside_points_0220(
            #         points_v, rect, Trv2c, P, Tr_list[ii], image_info['image_shape'][ii])
            #     points_v__.append(points_v_)

            #     # points_v = box_np_ops_spa.remove_outside_points(
            #     #     points_v, rect, Trv2c, P2, img_) #ori

        # points_v = points_v[points_v[:, 0] > 0]
        annos = info['annos']
        num_obj = len([n for n in annos['name'] if n != 'DontCare'])
        # annos = spa.filter_spa_anno(annos, ['DontCare'])
        dims = annos['dimensions'][:num_obj]
        loc = annos['location'][:num_obj]
        rots = annos['rotation_y'][:num_obj]
        ...
        # gt_boxes_camera = np.concatenate([loc, dims, rots[..., np.newaxis]],
        #                                  axis=1)
        # gt_boxes_lidar = box_np_ops_spa.box_camera_to_lidar_0220(
...
def create_spa_info_file(data_path,
                           pkl_prefix='spa',
                           with_plane=False,
                           save_path=None,
                           relative_path=True):
    """Create info file of spa dataset.
    ...
    spa_infos_train = get_spa_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, spa_infos_train, relative_path=False)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'spa info train file is saved to {filename}')
    mmcv.dump(spa_infos_train, filename)

    spa_infos_val = get_spa_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, spa_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'spa info val file is saved to {filename}')
    mmcv.dump(spa_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'spa info trainval file is saved to {filename}')
    mmcv.dump(spa_infos_train + spa_infos_val, filename)

    spa_infos_test = get_spa_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    print(f'spa info test file is saved to {filename}')
    mmcv.dump(spa_infos_test, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    Args:
        ...
        front_camera_id (int, optional): The referenced/front camera ID.
            Default: 2.
    """
    spa_infos = mmcv.load(info_path)

    for info in mmcv.track_iter_progress(spa_infos):
        pc_info = info['point_cloud']
        image_info = info['image']
        calib = info['calib']
        ...
        rect = calib['R0_rect']
        P_list = [calib['P2'], calib['P1'], calib['P2'], calib['P3'], calib['P4']]
        ...
        points_v_ = []
        for ii, P in enumerate(P_list):
            # points_v = box_np_ops_spa.remove_outside_points(points_v, rect, Trv2c, P2,
            #                                             image_info['image_shape'])
            points_v__ = box_np_ops_spa.remove_outside_points(points_v, rect, Trv2c, P,
                                                        image_info['image_shape'][ii])
            points_v_.append(points_v__)
        
        points_v_ = np.array(points_v_)
        points_v_ = np.vstack(points_v_)
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    spa_infos = mmcv.load(info_path)
    cat2Ids = [
        dict(id=spa_categories.index(cat_name), name=cat_name)
        for cat_name in spa_categories
    ]
    coco_ann_id = 0
    ...
    for info in mmcv.track_iter_progress(spa_infos):
        ...
        (height, width, _) = mmcv.imread(osp.join(info['image']['image_path'][0])).shape
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        for ii, final_coord in enumerate(final_coords):
            ...
            if mono3d and (repro_rec is not None):
                ...
                repro_rec['velo_cam3d'] = -1  # no velocity in spa
                ...
                repro_rec['attribute_name'] = -1  # no attribute in spa
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in spa_categories:
        return None
    cat_name = repro_rec['category_name']
    coco_rec['category_name'] = cat_name
    coco_rec['category_id'] = spa_categories.index(cat_name)
