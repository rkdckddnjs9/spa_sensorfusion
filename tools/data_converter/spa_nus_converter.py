...
def get_label_anno(label_path):
    ...
    name_map={'Car':'car', 'Truck':'truck', 'Bus':'bus', 'Pedestrian':'pedestrian','Bicyclist':'bicycle', 'Motorcycle':'motorcycle', 'Kickboard':'kickboard', 'Vehicle':'car'}
    ...
    annotations['dimensions'] = np.abs(np.array([[float(info) for info in x[9:12]]
                                          for x in content
                                          ]).reshape(-1, 3)[:, [1, 2, 0]])  #h, l, w -> l ,w, h 
...
def get_2d_boxes(nusc,
    ...
    for i in range(label_['name'].shape[0]):
        ...
        # for pts in 3dbox
        gt_boxes = np.array(ann_rec['translation'] + ann_rec['size'] + [label_['rotation_y'][i]+np.pi/2])
        ...
        projection_m = np.matmul(intrinsic, cam_lnfo['cam_intrinsic'][1])
